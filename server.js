...
app.post("/rate", async (req, res) => {
  ...
  try {
    ...
    const geminiTranslation = await translationText(
      message,
      language,
      "gemini-1.5-flash"
    );
    ...
    res
      .status(500)
      .json({ error: "An error occurred while processing the translations." });
  }
});
  ...
  try {
    ...
    try {
      ...
      for (const rank of rankings) {
        // Update existing translations with ranks and ratings
        const languageName = languageNames[rank.targetLanguage];
        await client.query(
          `UPDATE translations 
           SET rank = $1, 
               rating = $2
